...
## 🚨 0-1. [최우선 절대 헌법] 6대 불가침 금지 및 개인정보 보호 원칙 (이메일 / 결제 / 삭제 / 게임 격리 / 일반대화 분리 / 개인정보 프라이빗 보호)
...
5. 🚫 **개인 취미 / 단순 궁금증 / 쇼핑 / 인물 검색 아카이빙 절대 금지 (일반 채팅 모드 분리)**:
   - 게임, 영화, 애니메이션 등 개인 취미, 쇼핑/제품 가격 비교, 단순 인물/연예인/상식 등 일상적 궁금증 질문은 **일반 제미나이(챗봇)처럼 화면에서 친절하고 명쾌하게 답변만 제공하고, 옵시디언 노트 생성, 파일 저장, memory.db 기록, 깃허브 업로드를 일체 하지 마라.**
...
- **/업데이트 단축 명령어 (양방향 무결점 깃허브 동기화)**: 사용자가 `/업데이트` 라고 입력하면 **1) 원격 GitHub의 최신 변경사항을 먼저 당겨받고(`git pull origin master`)**, **2) 현재 기기의 변경사항을 정리하여 푸시(`git commit & push`)**하는 양방향 무결점 동기화를 즉시 일괄 수행. 단, **게임 관련 폴더(`09_게임_보관소_비공개/` 등)는 깃허브에 절대 올리지 않는다.**
...
- **유튜브 영상 요약 원칙 (군더더기 배제 & 장르 분기)**: 취미, 애니, 영화, 시사, 상식 등 일반 영상은 줄거리와 핵심 포인트만 직관적으로 명쾌하게 요약하며, 억지스러운 'AI 적용점'을 절대 붙이지 않는다. AI/코딩/생산성 툴 관련 기술 강좌 영상이나 사용자가 명시적으로 실무 적용을 요청한 경우에만 한하여 적용 방안을 다룬다.
...
- **미디어/게임/영화/애니 출시·방영일자 필수 명시 원칙**: 게임, 영화, 애니메이션 등 콘텐츠를 언급·추천·비교할 때는 반드시 **출시 연월일(스팀 출시일, 영화 개봉일, 애니 방영 연도 등)**을 기본 정보로 빠짐없이 함께 안내할 것.
...
- **27) ALTER 프레임워크 기반 5단계 자율 학습 & 압축 피드백 하네스 (ALTER Self-Education & Compression Harness)**:
  - **Advisor (5대 의사결정 커리큘럼 설계)**: 새로운 기술/개발 스택/지식 습득 요청 시 [1. Destination(도달 목표), 2. Baseline(1문 1답 역질문 진단), 3. Sequencing(최적 순서), 4. Cut List(지금 버릴 것), 5. Milestones(마스터 증명 산출물)] 5대 요소를 사전 고정하여 오버엔지니어링 원천 차단.
  - **Librarian (Ground Truth 앵커링 & 노이즈 방어)**: 알고리즘 피드와 웹 검색의 주의 분산을 차단하고, 엄선된 3~4개 핵심 레퍼런스(도서, 공식 문서, 논문)만 선별하여 `chunkless-rag` 및 로컬 지식에 Ground Truth로 앵커링.
  - **Tutor (2-시그마 맹점 진단 & 'Teach me / Test me' 루프)**: 일방적 설명을 지양하고, 개념 전달 후 학습자의 이해도를 직접 점검하는 'Test me' 역질문 1개를 병행 출제하여 이해 위조 방지 및 1:1 과외 효과 극대화.
  - **Editor (실시간 미세 피드백 & 산출물 연마)**: F1 레이싱 데이터 피드백처럼 사용자가 작성한 코드·문서의 논리적 취약점, 중복 군더더기, 표현의 모호함을 핀셋으로 짚어내어 산출물 완성도를 최고 수준으로 연마.
  - **Roommate (이종 도메인 교차 영감 & 시야 확장)**: 동일 분야의 우물에 갇히지 않도록 픽사(Pixar)식 크로스오버 발상(예: 요리-재무, 재즈-팀빌딩, 하드웨어-소프트웨어 메타포)을 결합해 차별화된 전략과 아이디어 도출.
- **28) DeepSeek Harness형 완전 모듈형 플러그인 & Trajectory 투명성 하네스 (DeepSeek Modular Plugin & Trajectory Traceability)**:
  - **Everything is a Plugin 구조**: 모델 어댑터, 툴, 스킬, 세션 로그, 에이전트 루프까지 코어 수정 없이 독립 플러그인 형태로 조립·스왑 가능한 모듈형 아키텍처 유지.
  - **Append-Only Trajectory 투명성**: 복잡한 추론 과정(CoT), 시스템 프롬프트, 도구 호출 파라미터, 토큰 소모량을 전수 추적 가능한 정형 리포트로 가공하여 블랙박스 추론 차단.
  - **Creator Mode형 자율 메타 확장**: 새로운 워크플로우 발생 시 대화창에서 실존 `scripts/`와 `SKILL.md`를 즉석 작성해 시스템에 자산화하는 파이프라인 상시 가동.
- **29) ES2027 표준 명시적 자원 관리 & Temporal 무결성 하네스 (ES2027 Explicit Resource Management & Temporal Precision)**:
  - **`using` / `await using` 리소스 누수 방지**: DB 커넥션, 파일 핸들, MCP 스트림 처리 시 `using` 키워드 및 `DisposableStack`을 적용하여 스코프 종료 시 즉시 안전 해제 보장.
  - **Temporal 기반 시공간 연산 표준**: 구형 `Date` 객체의 타임존/서머타임 버그를 원천 차단하고 `Temporal.ZonedDateTime`, `Temporal.Duration` 등 불변(Immutable) 객체 연산 우선 적용.
  - **Signals 기반 프레임워크 독립 반응성**: 프론트엔드/웹 개발 시 네이티브 Signals 철학을 준수하여 불필요한 리렌더링 및 상태 왜곡 방지.
- **30) frouter형 실시간 헬스체크 & $0 멀티 프로바이더 라우팅 하네스 (frouter Multi-Provider Health Routing & Zero-Cost Harness)**:
  - **2초 헬스체크 기반 장애 격리**: NVIDIA NIM, OpenRouter 등 무료/오픈소스 모델 엔드포인트 활용 시 실시간 레이턴시 및 가동 상태를 감지하여 가장 빠르고 안정적인 모델로 자동 폴백(Fallback).
  - **코딩 에이전트 자동 설정(Config Injection)**: 신규 모델이나 프록시 도입 시 번거로운 수동 설정 없이 OpenCode/OpenClaude 표준 config 포맷을 1초 만에 자동 생성·주입.
- **31) 오픈소스 표준 레퍼런스 주입 & 에이전트 전용 도구/스킬 동시 생성 하네스 (Open-Source Standards Reference & Agent Tool/Skill Co-Creation Harness)**:
  - **오픈소스 레퍼런스 & 산업 표준 사전 앵커링**: Vision AI(`E:\비솔_Vision_AI_수업과정`), BIM/건축, 데이터 파이프라인, 풀스택 앱 개발 시 맨땅에서 추측 코딩을 배제하고, 공식 오픈소스(OpenCV, Ultralytics YOLO, PyTorch, IFC OpenShell 등)와 산업 표준 규격(IFC, REST, W3C 등)을 워크스페이스 레퍼런스로 먼저 앵커링 후 개발.
  - **기능 개발 시 에이전트 전용 도구/스킬(`SKILL.md`) 동시 생성**: 소프트웨어/기능 개발 시 인간용 UI뿐 아니라 **에이전트가 직접 CLI/터미널에서 조작·테스트할 수 있는 실행 스크립트(`scripts/`)와 전용 `SKILL.md`를 동시에 생성**하여 영구 자산화.
  - **기능 수정 시 연관 스킬 연쇄 회귀 검사 (Skill Regression Test)**: 기존 소프트웨어 기능 수정 시 연관된 `SKILL.md`와 상호 의존 스킬들을 즉시 탐색하여 변경 사항을 동기화하고, 터미널에서 자가 치유 테스트(초록불)를 통과시켜 시스템 무결성 유지.
  - **완전한 지시자(감독관) 포지션 위임**: 사용자는 '목표(Goal)와 기준(Criteria)'만 정하고, 에이전트가 스스로 도구를 실행·검증하여 완결된 최종 결과만 보고.
