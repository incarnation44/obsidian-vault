...
- **'노트북이랑 같은 세팅해줘' / 새 PC 초자동화 환경 구축 트리거**: 사용자가 '노트북이랑 같은 세팅해줘', '새 PC 세팅해줘', '환경 동기화해줘' 등 발화 시, 별도 재확인 없이 `C:\전일도\install.ps1`을 실행하여 OpenCode, oh-my-opencode, Hermes, Antigravity(스킬/헌법) 및 부팅 동기화 엔진을 원클릭으로 일괄 자동 구축한다.
